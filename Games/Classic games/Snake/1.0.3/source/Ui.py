...
PIXEL_COLORS = {PIXEL_TYPES.EMPTY:QColor(0, 0, 0),
                PIXEL_TYPES.BARRIER:QColor(255, 0, 0),
                PIXEL_TYPES.SNAKE_BODY:QColor(0, 200, 0),
                PIXEL_TYPES.FOOD:QColor(0, 0, 255),
                PIXEL_TYPES.SNAKE_HEAD:QColor(0, 255, 0)}#游戏像素不同类型的颜色
UPDATE_INTERVAL = 170#更新时间间隔
GAME_RULES = """
上下左右键控制蛇的移动方向，绿色部分为蛇身，较亮的格子代表蛇头；
蓝色格子是食物，红色格子是障碍物；
在每次吃到食物时，蓝色、红色格子都会更新，请注意；
当碰到红色格子、游戏界面边缘或碰到蛇身时，游戏结束。
"""
PROGRAMME_INFO = """
作者：Happy Hu
版本：%s
...
class GameInterface(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.gameOver:
            ...
            qp.drawText(0, 0, MATRIX_SCREEN_SIZE.x, MATRIX_SCREEN_SIZE.y, Qt.AlignCenter, "GAME OVER")
        qp.end()
    def drawGamePixel(self, qp):
        qp.setBrush(PIXEL_COLORS[self.system.matrix[self.paintCursor]])
